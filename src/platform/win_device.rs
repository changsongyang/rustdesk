...
unsafe extern "system" {
    fn UpdateDriverForPlugAndPlayDevicesW(
        hwnd_parent: HWND,
        hardware_id: LPCWSTR,
        full_driver_path: LPCWSTR,
        install_flags: DWORD,
        b_delete_old_driver: *mut BOOL,
    ) -> BOOL;
}
...
pub unsafe fn install_driver(
    ...
    let mut class_guid: GUID = unsafe {
        std::mem::zeroed()
    };
    ...
    if unsafe {
        SetupDiCallClassInstaller(DIF_REGISTERDEVICE, *dev_info, &mut dev_info_data)
    } == FALSE {
...
pub unsafe fn uninstall_driver(
    ...
    if unsafe {
        SetupDiGetDeviceInfoListDetailW(*dev_info, &mut device_info_list_detail)
    } == FALSE {
        return Err(DeviceError::new_api_last_err(
    ...
    loop {
        if unsafe {
            SetupDiEnumDeviceInfo(*dev_info, device_index, &mut devinfo_data)
        } == FALSE {
        ...
        match unsafe {
            is_same_hardware_id(&dev_info, &mut devinfo_data, hardware_id)
        } {
            Ok(false) => {
                device_index += 1;
                continue;
        ...
        if unsafe {
            SetupDiCallClassInstaller(DIF_REMOVE, *dev_info, &mut devinfo_data)
        } == FALSE {
            return Err(DeviceError::new_api_last_err("SetupDiCallClassInstaller"));
        }
        ...
        if unsafe {
            SetupDiGetDeviceInstallParamsW(*dev_info, &mut devinfo_data, &mut device_params)
        } == FALSE
        {
            log::error!(
...
unsafe fn get_device_path(interface_guid: &GUID) -> Result<Vec<u16>, DeviceError> {
    ...
    for i in 0..(predicted_length - steps as u32) / 2 {
        if unsafe {
            *device_path_ptr.offset(i as _) == 0
        } {
            path.push(0);
            break;
        }
