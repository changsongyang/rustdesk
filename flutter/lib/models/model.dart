...
class FfiModel with ChangeNotifier {
  ...
  RxBool authDialogActive = false.obs;  // Track if an auth dialog is currently showing
  ...
  handleMsgBox(Map<String, dynamic> evt, SessionID sessionId, String peerId) {
    ...
    // Android-specific: Track auth dialog state to prevent closing it when first image arrives
    final isAuthDialog = isAndroid && authTypes.contains(type);
    if (isAuthDialog) {
      // Set flag to prevent dismissAll() from closing auth dialog in onEvent2UIRgba
      authDialogActive.value = true;
    }
  ...
  void showConnectedWaitingForImage(OverlayDialogManager dialogManager,
    ...
    waitForImageTimer = Timer(Duration(milliseconds: 1500), () {
      if (waitForFirstImage.isTrue && !isRefreshing) {
        ...
        final hasAuthDialog =
            dialogManager.hasDialogWithTag('$sessionId-password') ||
...
class FFI {
  ...
  void onEvent2UIRgba() async {
    ...
    if (ffiModel.waitForFirstImage.value == true) {
      ...
      // Only dismiss all dialogs if no auth dialog is active
      // Auth dialogs (wait-remote-accept-nook, etc.) should be closed by their own logic
      if (!ffiModel.authDialogActive.value) {
        dialogManager.dismissAll();
      }
      // Reset auth dialog flag
      ffiModel.authDialogActive.value = false;
