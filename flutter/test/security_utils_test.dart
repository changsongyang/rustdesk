  ...
  group('SecurityUtils', () {
    ...
    group('isValidIpAddress', () {
      ...
      test('validates IPv6 address', () {
        expect(
            SecurityUtils.isValidIpAddress(
    ...
    group('containsSqlKeywords', () {
      test('detects SELECT keyword', () {
        expect(
            SecurityUtils.containsSqlKeywords('SELECT * FROM users'), isTrue);
      });

      test('detects SQL injection attempt', () {
        expect(
            SecurityUtils.containsSqlKeywords("'; DROP TABLE users--"), isTrue);
      });
    ...
    group('validateSessionToken', () {
      test('validates valid token', () {
        expect(SecurityUtils.validateSessionToken(List.filled(32, 'a').join()),
            isTrue);
      });
      ...
      test('rejects token with spaces', () {
        expect(
            SecurityUtils.validateSessionToken(List.filled(16, 'a').join() +
                ' ' +
                List.filled(16, 'a').join()),
            isFalse);
      });
    ...
    group('maskSensitiveData', () {
      test('masks password correctly', () {
        expect(SecurityUtils.maskSensitiveData('password123456'),
            'pass*********6');
      });
